...
export class StoreComponent implements OnInit {
  ...
  // --- Custom mouse-based zone dragging ---
  onZoneMouseDown(event: MouseEvent, zone: Zone): void {
    // Prevent drag if resizing
    if (this.resizingZone) return;
    this.isDraggingZone = true;
    this.draggedZone = zone;
    ...
    document.addEventListener('mousemove', this.onZoneMouseMove);
    document.addEventListener('mouseup', this.onZoneMouseUp);
  }

  onZoneMouseMove = (event: MouseEvent) => {
    if (!this.isDraggingZone || !this.draggedZone) return;
    const mapContainer = document.querySelector('.map-container');
  ...
  onZoneMouseUp = (event: MouseEvent) => {
    if (this.isDraggingZone) {
      this.isDraggingZone = false;
      this.draggedZone = null;
      document.removeEventListener('mousemove', this.onZoneMouseMove);
      document.removeEventListener('mouseup', this.onZoneMouseUp);
    }
  }
}
